var app = {
  ...
  init: function() {
    ...
    $.ajax({
      ...
      error: function(error) {
        console.error("chatterbox: Failed to post user", error);
      }
    });
  ...
  renderMessage: function(message) {
    ...
    var $message = $("<br><span/>");
    $message.text(message.messageText).appendTo($chat);
    ...
    if (selectIndex === 0) {
      ...
      if (roomname) {
        ...
        $.ajax({
          ...
          error: function(error) {
            console.error("chatterbox: Failed to post room", error);
          }
        });
